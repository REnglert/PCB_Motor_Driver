...
void PI_update(void) {
    ...
    //  target_rpm
    //  motor_speed in raw encoder counts
    //  m_rpm = (m_S \cdot 60) / 64 count quadrature encoder
    //  e = (t - m)/t
     error = target_rpm - motor_speed: //((motor_speed * 60) / 64);
    ...
    /// TODO: Clamp the value of the integral to a limited positive range
     error_integral = error_integral % 3200;
    //Cap it at 3200
    ...
    // output error =  Kp * input error
    // output = error * gain
    int16_t output = ((Kp * error) + error_integral) / 32; // Change this!
